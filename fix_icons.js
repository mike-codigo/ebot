...
html = html.replace(
  ...
  '.wpp-icon { top: 6%; left: 6%; }'
);
html = html.replace(
  '.ebot-icon { bottom: 15%; right: 15%; animation-delay: -1.5s; }',
  '.ebot-icon { bottom: 6%; right: 6%; animation-delay: -1.5s; }'
);

// Add overflow: hidden to chat-wrapper to cleanly contain background and shadows safely
html = html.replace(
  '.chat-wrapper {\n  height: 400px;',
  '.chat-wrapper {\n  height: 400px;\n  overflow: hidden;'
);

// Update connection-line z-index
html = html.replace(
  /position: absolute;\n\s*top: 0; left: 0; width: 100%; height: 100%;\n\s*z-index: -1;/g,
  'position: absolute;\n  top: 0; left: 0; width: 100%; height: 100%;\n  z-index: 1;'
);

// Build regex to replace the integration-bg HTML block
const bgRegex = /<div class="integration-bg">[\s\S]*?<\/svg>\s*<\/div>/m;
...
const newBg = `<div class="integration-bg">
    ...
    <svg class="connection-line" viewBox="0 0 100 100" preserveAspectRatio="none">
      <path d="M 13,18 C 40,18 60,82 87,82" fill="none" stroke="rgba(135, 166, 48, 0.2)" stroke-width="1" />
...
html = html.replace(bgRegex, newBg);

fs.writeFileSync('index.html', html, 'utf8');
console.log('Icons and paths updated.');
