const fs = require('fs');
const mainJSContent = `// Main entry point
document.addEventListener('DOMContentLoaded', () => {
    ...
    <div class="wa-modal-overlay" id="waModal" style="pointer-events: none;">
      <div class="wa-modal-card" id="waModalCard" style="transform: scale(0.9) translateY(20px); transition: transform 0.4s cubic-bezier(0.4, 0, 0.2, 1);">
        ...
        <div style="display: flex; gap: 1rem; justify-content: center; flex-wrap: wrap;">
          <button class="btn btn-outline" id="waModalClose" style="padding: 0.875rem 1.75rem; flex: 1;">Voltar</button>
          <a href="https://wa.me/5546991130554?text=Ol%C3%A1%2C%20gostaria%20de%20falar%20com%20um%20especialista" target="_blank" class="btn btn-primary" id="waModalContinue" style="background: #27AE60; border-color: #27AE60; padding: 0.875rem 1.75rem; flex: 1;">Continuar</a>
        </div>
      </div>
    </div>
  \`;
  ...
  // Replace nodes to prevent event listener duplication causing double clicks
  document.querySelectorAll('.trigger-wa-modal').forEach(btn => {
...
console.log('js/main.js atualizado com correção do modal.');
